...
alias adbGetWlanInterface="$adb shell getprop wifi.interface | $dos2unix"
$adb shell echo >/dev/null 2>&1 && export androidWlanInterface=$(adbGetWlanInterface)
if [ -n "$androidWlanInterface" ];then
	alias adbGetWlanIP="{ $adb shell getprop dhcp.${androidWlanInterface/:*/}.ipaddress | \grep [0-9] || $adb shell ip -o addr show $androidWlanInterface | awk -F ' *|/' '/inet /{print\$4}'; } | $dos2unix"
...
function adbSetADBTcpPort {
	...
	local androidWlanInterface=$($adb shell getprop wifi.interface | $dos2unix)
